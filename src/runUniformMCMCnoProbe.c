...
/* This is a modification of the full model where the probeEffects are removed, i.e. p=0 for all p */
/* As a result, the sigma_p term also vanishes */
...
SEXP sigmaJump_mu; // Can be of variable size to account for changes to nChains. Use the following pointers to access the values
...
static void initialiseValues (double *S_hijk, double *sigma_S, double *c_hi, double *mu_h, double *sigma_mu, double *phi_hj) {
	...
	/* sigma_S */
	do {
	    sigma_S[chain] = rexp(1.0); // Just sample from an exp(1) distribution truncated at 5. The median of this is log(2) = ~0.7
	}
	while (sigma_S[chain] > 5);
	...
	for (h=0; h<H; h++) {
	    ...
	    for (j=0; j<J; j++) {
		cur_hj = h*J + j;
		p_inits[cur_chainKeep + I*K + 2 + I + H + cur_hj] = phi_hj[chain*H*J + cur_hj]; // phi_hj
	    }
	}
...
static void updateValues (double *S_hijk, double *sigma_S, double *c_hi, double *mu_h, double *sigma_mu, double *phi_hj) {
...
*******************************/
	...
	for (i=0; i<I; i++) {
		...
		pOld = exp(-0.5*( pow( (log( PM_hijk[cur_ik] - S_hijk[chain*I*K + cur_ik] ) - lambda_ik[cur_ik] ) / delta_ik[cur_ik], 2) + pow( (log( S_hijk[chain*I*K + cur_ik] / phi_hj[chain*H*J + cur_hj ] ) - c_hi[chain*I + i]) / sigma_S[chain], 2) ) ) / (S_hijk[chain*I*K + cur_ik]*(PM_hijk[cur_ik] - S_hijk[chain*I*K + cur_ik]));
		pNew =  exp(-0.5*( pow( (log( PM_hijk[cur_ik] - newValue ) - lambda_ik[cur_ik]) / delta_ik[cur_ik], 2) + pow( (log( newValue / phi_hj[chain*H*J + cur_hj ] ) - c_hi[chain*I + i]) / sigma_S[chain], 2) ) ) / (newValue*(PM_hijk[cur_ik] - newValue));
	...
	for (i=0; i<I; i++){
		...
		sumSq += pow( log(S_hijk[chain*I*K + cur_ik]) - c_hi[chain*I + i] - log(phi_hj[chain*H*J + cur_hj]), 2); // Increase the cumulative sum of squares
...
*********************************/
	...
	for (i=0; i<I; i++){
		...
		xBar_hi[i] += ( log(S_hijk[chain*I*K + cur_ik]) - log(phi_hj[chain*H*J + cur_hj]) ) / K;
	    }
...
******************/
	...
	for (chain=0; chain<nChains; chain++) {
		...
		for (k=0; k<K; k++) {
		    ...
		    xBar_hj[cur_hj] += (log(S_hijk[chain*I*K + cur_ik]) - c_hi[chain*I + i] ) / (double)(K_j[cur_j]*I_h[cur_h]); // Cumulative mean
		}
	    }
...
SEXP runUniformMCMCnoProbe (SEXP data, SEXP conditions, SEXP exons, SEXP bg_means, SEXP bg_sds, SEXP mcmcParam) {
    ...
    // And setup tau_mu with initial value of 0.2.
 ...
 **************************************************************************/
    ...
    initialiseValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, phi_hj);
...
**********************************/
    ...
    double temp_mu[nBurnin*nChains]; // Holds the values for sigma_mu
    int mu_done[nChains]; // A boolean variable indicating whether adapting is done
    float accRate_mu; // The acceptance rate for each. Use the last 100 samples to estimate this
    float mean_mu; // Gets the mean for each normal approximation
    double var_mu=0; // Get the var for each normal approximation
    int t; // A local variable for iterating through during the adapting phase
    int nextAdapt = 100; // Used to set the next iteration to adapt tau_mu 
	...
	updateValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, phi_hj);
 ...
 *********************************************************/
     ...
     * The total estimable parameters in the model is: totParam = I*K + I + H + K + H*J + 2
     * Signal Level: I*K + 1
	...
	for (j=0; j<J; j++) {
	    ...
	    SET_STRING_ELT(paramNames, I*K + 2 + I + H + h*J + j, mkChar(temp)); // phi_hj
	}
    }
	...
	if (iter==keep[nextKeep]) {
		...
		for (h=0; h<H; h++) {
		    ...
		    for (j=0; j<J; j++) {
			cur_hj = h*J + j;
			p_output[cur_chainKeep + I*K + 2 + I + H + cur_hj] = phi_hj[chain*H*J + cur_hj]; // phi_hj
		    }
		}
	    }
