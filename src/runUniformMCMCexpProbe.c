/* A variant on the model where the probe effects term is modelled
...
//static const double scanLimit = 16*M_LN2; 
...
static void initialiseValues (double *S_hijk, double *sigma_S, double *c_hi, double *mu_h, double *sigma_mu, double *p_k, double *phi_hj) {
	...
	for (i=0; i<I; i++) {
	    ...
	    c_hi[chain*I + i ] = log(S_hijk[chain*I*K + i*K + samp]); // Take the log of the signal estimate at this probe on chip 'i' as the chip effect
	    cur_h = p_conditions[i]-1;
	...
	for (k=0; k<K; k++) {
	  ...
	  //  cur_hj = (p_conditions[i] - 1)*J + p_exons[k] - 1;
	    // Find each probe effect as a random negative exponential
...
*/
	...
	for (h=0; h<H; h++) {
	    ...
	    for (j=0; j<J; j++) {
		cur_hj = h*J + j;
		p_inits[cur_chainKeep + I*K + 2 + I + H + K + cur_hj] = phi_hj[chain*H*J + cur_hj]; // phi_hj
	    }
	}
...
static void updateValues (double *S_hijk, double *sigma_S, double *c_hi, double *mu_h, double *sigma_mu, double *p_k, double *phi_hj) {
...
*/
		...
		pOld = exp(-0.5*( pow( (log( PM_hijk[cur_ik] - S_hijk[chain*I*K + cur_ik] ) - lambda_ik[cur_ik] ) / delta_ik[cur_ik], 2) + pow( (log( S_hijk[chain*I*K + cur_ik] / phi_hj[chain*H*J + cur_hj ] ) - c_hi[chain*I + i] - p_k[chain*K + k]) / sigma_S[chain], 2) ) ) / (S_hijk[chain*I*K + cur_ik]*(PM_hijk[cur_ik] - S_hijk[chain*I*K + cur_ik]));
		pNew =  exp(-0.5*( pow( (log( PM_hijk[cur_ik] - newValue ) - lambda_ik[cur_ik]) / delta_ik[cur_ik], 2) + pow( (log( newValue / phi_hj[chain*H*J + cur_hj ] ) - c_hi[chain*I + i] - p_k[chain*K + k]) / sigma_S[chain], 2) ) ) / (newValue*(PM_hijk[cur_ik] - newValue));
...
*********************************/
	...
	pOld = 1;
	pNew = 1;
	sigma_h = sigma_S[chain] / SQRT_K; // This is used in all steps & is obtained after sigma_S has been updated
	 ...
	 ************/
	/* OLD METHOD */
	// This may be a little inefficient, but it works...
/*	do {
...
/*	    do {
	    ...
	    while (mu_h[chain*H + h] <=0 || mu_h[chain*H + h] >= scanLimit); // Ensure that the random sampled value is within the prescribed limits
*/
	    /* NEW METHOD */
...
/*	    do {
	    ...
	    while (c_hi[chain*I + i] <=0 || c_hi[chain*I + i] >= scanLimit); // Ensure the values are within the correct limits
*/
	    /* NEW METHOD */
...
*************************/
	...
	// Using the negative exponential prior
	// We need the two values xBar_k & sigma_k

	GetRNGstate();

	sigma_k = sigma_S[chain] / SQRT_I; // Calculate once sigma_S has been updated
//	V_k = pow(sigma_p[chain]*sigma_k, 2) / ( pow(sigma_p[chain], 2) + pow(sigma_k, 2));
	...
	for (k=0; k<K; k++) { // for each probe
		...
		xBar_k[k] += (log( S_hijk[chain*I*K + cur_ik] ) - c_hi[chain*I + i] - log( phi_hj[chain*H*J + cur_hj ] )) / (double)I;
	    }

	    // Sample the new value truncated at zero
	    p_k[chain*K + k] = r_righttruncnorm(0.0, xBar_k[k] + pow(sigma_k, 2), sigma_k);
...
******************/
	...
	for (chain=0; chain<nChains; chain++) {
		...
		for (k=0; k<K; k++) {
		    ...
		    xBar_hj[cur_hj] += (log(S_hijk[chain*I*K + cur_ik]) - c_hi[chain*I + i] - p_k[chain*K + k]) / (double)(K_j[cur_j]*I_h[cur_h]); // Cumulative mean
		}
	    }
...
SEXP runUniformMCMCexpProbe (SEXP data, SEXP conditions, SEXP exons, SEXP bg_means, SEXP bg_sds, SEXP mcmcParam) {
...
//    initialiseValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, p_k, sigma_p, phi_hj);
    initialiseValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, p_k, phi_hj);
...
//    double var_mu=0, var_p; // Get the var for each normal approximation
    double temp_mu[nBurnin*nChains];
    int mu_done[nChains];
    float accRate_mu;
    float mean_mu;
    double var_mu=0;
...
//	updateValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, p_k, sigma_p, phi_hj);
      updateValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, p_k, phi_hj);
...
//    SET_STRING_ELT(paramNames, I*K + 2 + I + H + K, mkChar("sigma_p"));
	...
	for (j=0; j<J; j++) {
	    ...
	    SET_STRING_ELT(paramNames, I*K + 2 + I + H + K + h*J + j, mkChar(temp)); // phi_hj
	}
    }
...
//		p_output[cur_chainKeep + I*K + 2 + I + H + K] = sigma_p[chain]; // sigma_p
		...
		for (h=0; h<H; h++) {
		    ...
		    for (j=0; j<J; j++) {
			cur_hj = h*J + j;
			p_output[cur_chainKeep + I*K + 2 + I + H + K + cur_hj] = phi_hj[chain*H*J + cur_hj]; // phi_hj
		    }
		}
