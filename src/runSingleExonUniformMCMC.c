...
/* Modified for a single exon in Jan 2011 */
...
/* The modification for a single exon is that phi=1 for all iterations */
/* It has been removed from all steps of the calculations as a result */
...
void setSingleExonDimensions (SEXP conditions, SEXP exons){
...
void initialiseSingleExonValues (double *S_hijk, double *sigma_S, double *c_hi, double *mu_h, double *sigma_mu, double *p_k, double *sigma_p, double *phi_hj) {
	...
	for (i=0; i<I; i++) {
		...
		cur_ik = i*K + k;
		do {
		    S_hijk[chain*I*K + cur_ik] = PM_hijk[cur_ik] - rlnorm(lambda_ik[cur_ik], delta_ik[cur_ik]);
		}
		while (S_hijk[chain*I*K + cur_ik]<0);
	    }
	}

	/* sigma_S */
	sigma_S[chain] = runif(0,10);

	/* c_hi & mu_h */
	for (i=0; i<I; i++) {
	    samp = (int)runif(0, K); // Choose a random integer between 0 & K for the current I
	    c_hi[chain*I + i ] = log(PM_hijk[i*K + samp]); // Take the log of the signal at this probe on chip 'i' as the chip effect
	    cur_h = p_conditions[i]-1;
	    mu_h[chain*H + cur_h] += c_hi[chain*I + i] / I_h[cur_h]; // Take the average of the sampled c_hi values as the mu_h estimate
	}
	...
	for (k=0; k<K; k++) {
	    for (i=0; i<I; i++) {
		cur_ik = i*K + k;
		cur_hj = p_conditions[i] - 1;
		// Find each probe effect as the mean of log(S_hijk) - c_hi - log(phi_hj)
		p_k[chain*K + k] += (log(S_hijk[chain*I*K + cur_ik]) - c_hi[chain*I + i]) / I;
	    }
...
void updateSingleExonValues (double *S_hijk, double *sigma_S, double *c_hi, double *mu_h, double *sigma_mu, double *p_k, double *sigma_p) {
    ...
    /* Should most of these be declared globally ??? */
    /* It makes sense, as then they are only defined once, which should make things faster */
    ...
    double sigma_h;    // This will need to be K_h for the mixture model, and sigma_h will have length 'H'
    double V_h[H], muHat_h[H]; // Used for updating mu_h
    double V_hi, thetaHat_hi[I]; // Used for updating c_hi

    // Those for step 3
    double xBar_k[K];
    double sigma_k, V_k, theta_k;
...
*******************************/
	...
	for (i=0; i<I; i++) {
		...
		cur_hj = p_conditions[i] - 1; // The calculated index for the current h & j, (will always be 'h')

		do {
		    newValue = PM_hijk[cur_ik] - rlnorm(lambda_ik[cur_ik], delta_ik[cur_ik]); // The proposed new value
		}
		while (newValue<0); // Ensure 0 < newValue < PM_hijk

		// Calculate the values of the pdf for the old & new values
		pOld = exp(-0.5*( pow( (log( PM_hijk[cur_ik] - S_hijk[chain*I*K + cur_ik] ) - lambda_ik[cur_ik] ) / delta_ik[cur_ik], 2) + pow( (log( S_hijk[chain*I*K + cur_ik] ) - c_hi[chain*I + i] - p_k[chain*K + k]) / sigma_S[chain], 2) ) ) / (S_hijk[chain*I*K + cur_ik]*(PM_hijk[cur_ik] - S_hijk[chain*I*K + cur_ik]));
		pNew =  exp(-0.5*( pow( (log( PM_hijk[cur_ik] - newValue ) - lambda_ik[cur_ik]) / delta_ik[cur_ik], 2) + pow( (log( newValue ) - c_hi[chain*I + i] - p_k[chain*K + k]) / sigma_S[chain], 2) ) ) / (newValue*(PM_hijk[cur_ik] - newValue));
		...
		if (samp == 1) S_hijk[chain*I*K + cur_ik] = newValue;
	...
	for (i=0; i<I; i++){
		...
		cur_ik = K*i + k; // The calculated index for the current i & k;
		cur_hj = p_conditions[i] - 1; // The calculated index for the current h & j

		sumSq += pow( log(S_hijk[chain*I*K + cur_ik]) - c_hi[chain*I + i] - p_k[chain*K + k], 2); // Increase the cumulative sum of squares
...
*********************************/
	...
	pOld = 1;
	pNew = 1;
	sigma_h = sigma_S[chain] / sqrt(K); // This is used in all steps & is obtained after sigma_S has been updated
	...
	for (i=0; i<I; i++){
	    ...
	    for (k=0; k<K; k++) {
		cur_ik = K*i + k; // The calculated index for the current i & k;
		xBar_hi[i] += ( log(S_hijk[chain*I*K + cur_ik]) - p_k[chain*K + k] ) / K;
	    }
	 ...
	 ************/
	do {
	    newValue = rnorm( sigma_mu[chain], tau_mu[chain] ); // Generate a new value within the truncation limits
	}
	...
	for (h=0; h<H; h++) {
	    pOld *= exp(-0.5*((I_h[h]-1)*log( pow(sigma_h, 2) + pow(sigma_mu[chain], 2) ) + sumSq_h[h] / ( pow(sigma_h, 2) + pow(sigma_mu[chain], 2) ) ));
	    pNew *= exp(-0.5*((I_h[h]-1)*log( pow(sigma_h, 2) + pow(newValue, 2) ) + sumSq_h[h] / ( pow(sigma_h, 2) + pow(newValue, 2) ) ));
	}
	...
	for (h=0; h<H; h++) {
	    V_h[h] = (pow(sigma_h, 2) + pow(sigma_mu[chain], 2)) / I_h[h]; // Posterior variance for each condtion
	    do {
		mu_h[chain*H + h] = rnorm(muHat_h[h], sqrt(V_h[h])); // Update the value for mu_h
	    }
	    while (mu_h[chain*H + h] <=0 || mu_h[chain*H + h] >= 16*log(2)); // Ensure that the random sampled value is within the prescribed limits
	}

	/********
	 * c_hi *
	 ********/
	V_hi = pow(sigma_mu[chain]*sigma_S[chain], 2) / ( K*pow(sigma_mu[chain], 2) + pow(sigma_S[chain], 2));
	for (i=0; i<I; i++) {

	    cur_h = p_conditions[i] - 1;
	    thetaHat_hi[i] = V_hi*(K*xBar_hi[i]/pow(sigma_S[chain], 2) + mu_h[chain*H + cur_h ]/pow(sigma_mu[chain], 2));
	    do {
		c_hi[chain*I + i] = rnorm(thetaHat_hi[i], sqrt(V_hi)); // Sample each new value
	    }
	    while (c_hi[chain*I + i] <=0 || c_hi[chain*I + i] >= 16*log(2)); // Ensure the values are within the correct limits
...
*************************/
	...
	sigma_k = sigma_S[chain] / sqrt((double)I); // Calculate once sigma_S has been updated
	V_k = pow(sigma_p[chain]*sigma_k, 2) / ( pow(sigma_p[chain], 2) + pow(sigma_k, 2));
	...
	for (k=0; k<K; k++) { // for each probe
		...
		cur_ik = i*K+k;
		cur_hj = p_conditions[i] - 1;
		xBar_k[k] += (log( S_hijk[chain*I*K + cur_ik] ) - c_hi[chain*I + i]) / (double)I;
	    }
	    
	    theta_k = V_k*xBar_k[k] / pow(sigma_k, 2); // The posterior mean
	 ...
	 ***********/
	do {
	    newValue = rnorm( sigma_p[chain], tau_p[chain] );
	}
	while(newValue<=0 || newValue >=10);

	// These values have been checked as correct by using R.
	...
	pOld = 1;
	for (k=0; k<K; k++) {
	    pOld *= exp(-0.5*(log(pow(sigma_k, 2) + pow(sigma_p[chain], 2)) + pow(xBar_k[k], 2)/(pow(sigma_k, 2) + pow(sigma_p[chain], 2))));
	    pNew *= exp(-0.5*(log(pow(sigma_k, 2) + pow(newValue, 2)) + pow(xBar_k[k], 2)/(pow(sigma_k, 2) + pow(newValue, 2))));
	}
...
SEXP runSingleExonUniformMCMC (SEXP data, SEXP conditions, SEXP exons, SEXP bg_means, SEXP bg_sds, SEXP mcmcParam) {
 ...
 **************************************************************/
    ...
    setSingleExonDimensions(conditions, exons); // Call the above function to set H, I, J, K, I_h & K_J
 ...
 **************************************************************************/
    ...
    initialiseSingleExonValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, p_k, sigma_p, phi_hj);
...
**********************************/
	...
	updateSingleExonValues(S_hijk, sigma_S, c_hi, mu_h, sigma_mu, p_k, sigma_p);
 ...
 *********************************************************/
    ...
    int totParam = I*K + I + H + K + H*J + 3; // This is the total number of parameters in the model
